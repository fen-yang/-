...
| 分支类型 | 分支名称 | 用途 |
|----------|----------|------|
| 主分支 | `main` | 存放稳定版本，仅接受来自 `dev` 的合并 |
| 开发分支 | `dev` | 日常集成分支，所有 feature 分支的合并目标 |
| 功能分支 | `feature/xxx` | 用于开发新功能或编写新文档，从 `dev` 切出，完成后合并回 `dev` |
...
| type | 说明 | 示例 |
|------|------|------|
| `docs` | 文档新增或更新 | `docs: 添加WBS分解文档` |
| `feat` | 功能开发 | `feat: 实现扫码签到功能` |
| `fix` | 问题修复 | `fix: 修复报名人数统计错误` |
| `chore` | 初始化、配置变更 | `chore: 初始化项目目录结构` |
| `merge` | 合并冲突解决 | `merge: 解决conflict.md冲突` |

**合并规则**：

| 合并方向 | 审批人 | 要求 |
|----------|--------|------|
| `feature/*` → `dev` | 计划负责人 | 无冲突，提交信息规范 |
| `dev` → `main` | 项目经理 | 所有测试通过，文档齐全 |

**版本标记规则**：

- 使用 Git 标签（tag）进行版本标记，格式：`v<主版本>.<次版本>.<补丁版本>`
